...
// InitProgressbar creates and returns a progress bar that tracks all the progress.
...
// AddToTotal update total progress request count
...
// Wait ensures that a progress bar's total count is up-to-date if during an enumeration there were uncompleted requests and
